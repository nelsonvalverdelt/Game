#pragma once
#include "../Game/ArrowsController.h";
#include <Windows.h>
#include <mmsystem.h>
...
namespace Game {
	...
	public:
		...
		UIGame(void)
		{
			...
			//Flechas Superiores de colores  y direcciones 4x4
			arrows = gcnew Bitmap("FlechasSprite.png");
			//Flechas Inferiores estáticas blancas 4x1
...
#pragma endregion
	...
	private: System::Void timerArrows_Tick(System::Object^  sender, System::EventArgs^  e) {
		...
		// Ésto permitirá dibujar nuestro fondo del juego a base de la velocidad de éste timer
		// y evitar que deje rastros de la imagen de las flechas anteriores 
		bufferedGraphics->Graphics->DrawImage(background, 0, 0);

		// Velocidad establecida dentro de las propiedades del timer Arrows + 5
		// ésto indicará que serán números divisibles por 5
		speed = speed + 5;
	
		//Indicamos, si el tiempo del reloj del timer "timerGame" cumple el Minuto = 60 seg
		if (time <= 60) {
		
			// cada 120 mili segundos establecidos dentro del timer permitirá realizar una acción
			// ésta acción será especificar el tiempo de salida de las flechas de colores y direcciones desde el eje Y= 0 por columna
			if (speed % 120 == 0) {

				//Aplicamos un random de solo dos números
				// Los números random serán 0 y 1
				// Ésto asumiran al cero = false y 1 = true
				outputArrowsCol1 = rand() % 2;
				outputArrowsCol2 = rand() % 2;
				outputArrowsCol3 = rand() % 2;
				outputArrowsCol4 = rand() % 2;

				// Ésto es  una restricción de salidas de flechas por columnas dentro de una condidición
				// Lo que no permitirá salir siempre en paralelo con el resto de flechas 
				...
				}else if (outputArrowsCol2 == 1 && outputArrowsCol1 == 0 && outputArrowsCol3 == 0 && outputArrowsCol4 == 0) {
					arrowController->alignArrowsPositionX(arrows, 2);
				}else if (outputArrowsCol3 == 1 && outputArrowsCol1 == 0 && outputArrowsCol2 == 0 && outputArrowsCol4 == 0) {
					arrowController->alignArrowsPositionX(arrows, 3);
				}else if (outputArrowsCol4 == 1 && outputArrowsCol3 == 0 && outputArrowsCol1 == 0 && outputArrowsCol2 == 0) {
					arrowController->alignArrowsPositionX(arrows, 4);
				}

			}
			
			//Dibuja las flechas de lluvia por colores y direcciones y por columnas
			...
			//Puntos establecidos en un label
	...
	//Setear todos los datos a cero en el reporte de los GroupBox
	void restartGame() {
